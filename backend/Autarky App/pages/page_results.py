...
def visualize_results() -> None:
    ...
    seasonal_files = {k: os.path.join(results_path, v) for k, v in season_map.items()}
    seasonal_files_exist = all(os.path.isfile(path) for path in seasonal_files.values())
