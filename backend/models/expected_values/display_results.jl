...
println("  System Components: ",
    ...
    has_mini_hydro ? "Mini-Hydro, " : "",
    has_battery ? "Battery Bank, " : "",
    has_diesel_generator ? "Backup Generator, " : "",
    has_biogas_generator ? "Biogas Generator" : "")
...
if has_mini_hydro
    println("  Mini-Hydro Capacity: ", round(value(model[:hydro_units]) * hydro_nominal_capacity, digits=2), " kW")
end
if has_battery
...
if has_biogas_generator
    println("  Biogas Generator Capacity: ", round(value(model[:biogas_units]) * biogas_nominal_capacity, digits=2), " kW")
end
...
if has_mini_hydro
    total_hydro_production = sum(season_weights[s] * value(model[:hydro_production][t, s]) for t in 1:T, s in 1:S)
    println("  Total Annual Mini-Hydro Production: ", round(total_hydro_production / 1000, digits=2), " MWh/year")
end
...
if has_biogas_generator
    total_biogas_production = sum(season_weights[s] * value(model[:biogas_production][t, s]) for t in 1:T, s in 1:S)
    println("  Total Annual Biogas Production: ", round(total_biogas_production / 1000, digits=2), " MWh/year")
    total_biogas_fuel = sum(season_weights[s] * (value(model[:biogas_production][t, s]) / biogas_fuel_lhv) for t in 1:T, s in 1:S)
    println("  Total Biogas Fuel Consumption: ", round(total_biogas_fuel, digits=2), " units/year")
end
...
if maximum_lost_load > 0.0
    ...
    lost_load_share = total_lost_load / sum(season_weights[s] * load[t, s] for t in 1:T, s in 1:S)
    println("  Total Lost Load: ", round(total_lost_load / 1000, digits=2), " MWh/year")
    println("  Lost Load Share: ", round(lost_load_share * 100, digits=2), " %")
end

# Renewable Penetration: Solar + Wind + Mini-Hydro only
if has_solar || has_wind || has_mini_hydro
    ...
    if has_mini_hydro
        total_renewables += sum(season_weights[s] * value(model[:hydro_production][t, s]) for t in 1:T, s in 1:S)
    end
    ...
    if has_biogas_generator
        total_generation += sum(season_weights[s] * value(model[:biogas_production][t, s]) for t in 1:T, s in 1:S)
    end
    if has_grid_connection
        total_generation += sum(season_weights[s] * value(model[:grid_import][t, s]) for t in 1:T, s in 1:S)
    end
