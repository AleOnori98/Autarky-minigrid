...
# DEBUG: Print season weights
println("Season Weights: ", season_weights)
...
# === Forecast Errors ===
forecast_errors = model_uncert["forecast_errors"]

if forecast_errors["load"]
    load_errors = import_time_series(joinpath(ts_dir, "load_forecast_errors.csv"), num_seasons, has_seasonality)
end

if forecast_errors["solar_pv"]
    solar_errors = import_time_series(joinpath(ts_dir, "solar_pv_forecast_errors.csv"), num_seasons, has_seasonality)
end

if forecast_errors["wind_turbine"]
    wind_errors = import_time_series(joinpath(ts_dir, "wind_turbine_forecast_errors.csv"), num_seasons, has_seasonality)
end

if forecast_errors["mini_hydro"]
    hydro_errors = import_time_series(joinpath(ts_dir, "mini_hydro_forecast_errors.csv"), num_seasons, has_seasonality)
end

# === Probabilistic Config (optional) ===
if haskey(model_uncert, "probabilistic_config")
    prob_config = model_uncert["probabilistic_config"]
    if has_grid_connection
        outage_duration = prob_config["outage_duration"]
        outage_probability = prob_config["outage_probability"]
    end
end
...
if seasonality
    ...
    local load_error_path = joinpath(@__DIR__, "..", "inputs", "errors", "load_errors.csv")
    load_errors[1] = CSV.read(load_error_path, DataFrame)

    # Load solar prediction errors
    local solar_error_path = joinpath(@__DIR__, "..", "inputs", "errors", "solar_errors.csv")
